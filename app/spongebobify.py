...
def spongebobify(text):
    ...
    for count, value in enumerate(text):
        res += value.upper() if count % 2 == 0 else value.lower()
    return res


def create_gif(image, text_x_pos, text_y_pos, text_on_image, font, img_io):
...
def create_image(image, text_x_pos, text_y_pos, text_on_image, font, img_io):
...
def create_meme(
    ...
    img_io = BytesIO()
    return (
        create_gif(image, text_x_pos, text_y_pos, text_on_image, font, img_io)
        if ".gif" in image_path
        else create_image(image, text_x_pos, text_y_pos, text_on_image, font, img_io)
    )
